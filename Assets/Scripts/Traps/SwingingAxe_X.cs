...
public class SwingingAxe_X : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        chainTransform.rotation = Quaternion.Euler(0, 0, startSwingArch);
    ...
    /// <summary>
    /// Check the Axe position on the x axis and if it exceeds 45 degrees or less than -45 degrees, set the swing bool
    /// </summary>
    private void CheckAxePosition()
    {
        ...
        if(chainTransform.rotation.eulerAngles.z > swingArch && 
           chainTransform.rotation.eulerAngles.z < 180f)
        {
            chainTransform.transform.rotation = Quaternion.Euler(0, 0, swingArch - 1);
            canSwingAxe = true;
        }
        else if(chainTransform.rotation.eulerAngles.z < (360 - swingArch) &&
                chainTransform.rotation.eulerAngles.z > 180f)
        {
            chainTransform.transform.rotation = Quaternion.Euler(0, 0, (360 - swingArch) + 1);
            canSwingAxe = true;
        }
    }
